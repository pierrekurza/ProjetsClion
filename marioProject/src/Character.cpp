...
float Character::getDirX() const { return dirX; }

float Character::getDirY() const { return dirY; }
...
void Character::setDirX(float v) { dirX = v; }

void Character::setDirY(float v) { dirY = v; }
        ...
        if (xLU >= 0 && xRD < MAX_X && yLU >= 0 && yRD < MAX_Y) {
            ...
            if (dirX > 0) {
                ...
                if ((map.getTile(yLU, xRD) > BLANK_TILE && map.getTile(yLU, xRD) != POWERUP_COIN &&
                    ...
                    powerUp(yRD, xRD, map, sound);
                    powerUp(yLU, xRD, map, sound);
                }
            }
                //////////////////////////////
                // Pareil à gauche ///////////
                ...
                if ((map.getTile(yLU, xLU) > BLANK_TILE && map.getTile(yLU, xLU) != POWERUP_COIN &&
                    ...
                    powerUp(yRD, xLU, map, sound);
                    powerUp(yLU, xLU, map, sound);
                }
            }
        }
...
void Character::mapCollideVertical(Tilemap &map, KeyboardInput &input, GameSound &sound) {
            ...
            if(dirY < 0) {
                if((map.getTile(yLU, xLU) > BLANK_TILE && map.getTile(yLU, xLU) != POWERUP_COIN &&
                   ...
                   (map.getTile(yLU, xRD) > BLANK_TILE && map.getTile(yLU, xRD) != POWERUP_COIN &&
                    ...
                    itemBlock(yLU, xLU, map, sound);
                    itemBlock(yLU, xRD, map, sound);
...
int Character::updateCharacter(KeyboardInput &input, Tilemap &map, GameSound &sound) {
    ...
    if (!deathTimer) {
        ...
        mapCollideVertical(map, input, sound);
        isOnLadder(map);
